...
export default {
  ...
  methods: {
    ...
    resetPassword(){
      this.$request.put('/api/rePassword',this.forgetUserForm).then(res =>{
        if (res.code === 200){
          this.$message.success({
            message: '重置密码成功!重置密码:' + res.data.password + ' 请尽快修改密码!',
            duration: 5000 // 显示时长为3000毫秒，即3秒
          });
    ...
    login() {
      this.$refs['loginRef'].validate((valid) => {
        if (valid) {
          // 验证通过
          this.$request.post('/api/login', this.user).then(res => {
            if (res.code === 200) {
              this.$router.push('/')
              this.$message.success('登录成功')
              localStorage.setItem("StudentSystem-user", JSON.stringify(res.data))  // 存储用户数据
            } else {
              this.$message.error(res.msg)
            }
          })
