  ...
  methods: {
    ...
    delBatch() {
      ...
      this.$confirm('您确认批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/api/news/delete/batch', { data: this.ids }).then(res => {
    ...
    del(id) {
      this.$confirm('您确认删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/api/news/delete/' + id).then(res => {
          if (res.code === 200) {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
    ...
    setRichText() {
      this.$nextTick(() => {
        ...
        this.editor.config.uploadImgServer = this.$baseUrl + '/api/file/editor/upload'
        ...
        this.editor.config.uploadVideoServer = this.$baseUrl + '/api/file/editor/upload'
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          ...
          this.$request({
            url: this.form.id ? '/api/news/update': '/api/news/add',
    ...
    load(pageNum) {  // 分页查询
      ...
      this.$request.get('/api/news/selectByPage', {
        params: {
